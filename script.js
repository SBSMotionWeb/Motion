    ...
    setTimeout(() => {
        ...
        const p3Tl = gsap.timeline({
            scrollTrigger: {
                trigger: "#page-3",
                start: "top top", 
                end: "bottom 60%", // 애니메이션 종료 시점을 훨씬 위로 당겨서 빠르게 끝냅니다.
                scrub: 0.5,
            }
        });

        p3Tl.to(".p3-moving-content", { x: "-30vw", y: "-38vh", scale: 0.6, duration: 0.5 }); // duration 단축
        p3Tl.to(".p3-curriculum-container", { autoAlpha: 1, y: -20, duration: 0.3 }, "-=0.2");

        // [3배 속도 적용] 각 문구 강조 시간 단축 (0.6 -> 0.2)
        gsap.utils.toArray(".p3-line").forEach((line) => {
            p3Tl.to(line, { opacity: 1, duration: 0.2, ease: "none" });
        });
